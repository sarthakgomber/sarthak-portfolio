...
const PROJECTS = [
  ...
  },
  {
    ...
    description: 'A travel digital ID system for tourist safety. Includes real-time location tracking, panic button alerts, and a web-based admin dashboard for authorities.',
...
function ProjectCard({ project, index }) {
  ...
  return (
    <div ref={cardRef} onMouseMove={onMouseMove} className="glass"
      style={{ padding: '2rem', transition: 'border-color 0.3s ease, box-shadow 0.3s ease' }}
      onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.28)' }}
      onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.18)' }}
    >
      <div className="project-card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.2rem', gap: '1rem' }}>
        <div>
          <p className="chrome-label" style={{ marginBottom: '0.4rem' }}>Project {String(index + 1).padStart(2, '0')}</p>
          <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 'clamp(1.4rem, 3vw, 1.8rem)', fontWeight: 500, color: 'rgba(255,255,255,0.92)', letterSpacing: '0.05em' }}>
            {project.name}
          </h3>
        </div>
        <a href={project.github} target="_blank" rel="noopener noreferrer"
          className="chrome-btn" style={{ padding: '0.45rem 1rem', fontSize: '0.62rem', flexShrink: 0 }}>
          GitHub ↗
        </a>
      </div>
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.72rem', color: 'rgba(255,255,255,0.35)', letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: '1rem' }}>
        {project.tagline}
      </p>
      <div className="chrome-line" style={{ marginBottom: '1.2rem' }} />
      <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.88rem', fontWeight: 300, lineHeight: 1.8, color: 'rgba(255,255,255,0.55)', marginBottom: '1.4rem' }}>
        {project.description}
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.4rem', marginBottom: '1.4rem' }}>
        {project.highlights.map(h => (
          <div key={h} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.78rem', color: 'rgba(255,255,255,0.5)' }}>{h}</span>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
        {project.tech.map(t => <span key={t} className="chrome-tag">{t}</span>)}
      </div>
    </div>
  )
}
